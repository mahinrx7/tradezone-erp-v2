...
def view_expenses():
    ...
    query = Expense.query.options(joinedload(Expense.site))
    ...
    work_query = WorkEntry.query.options(joinedload(WorkEntry.labour))
    if site_id and site_id != "all":
        work_query = work_query.filter_by(site_id=site_id)
    work_entries = work_query.all()

    total_expense = sum(float(e.amount or 0) for e in expenses_data)
    labour_total = sum(
        float(entry.hours or 0) * float(entry.labour.hourly_rate or 0)
        for entry in work_entries if entry.labour
    )
...
def add_expense():
    ...
    sites = Site.query.all()
    return render_template("add_expense.html", sites=sites)
...
def export_expenses_excel():
    expenses_data = Expense.query.options(joinedload(Expense.site)).all()
...
def export_expenses_pdf():
    ...
    data = [["Site", "Category", "Description", "Amount", "Date"]]
    for expense in expenses_data:
        data.append([
